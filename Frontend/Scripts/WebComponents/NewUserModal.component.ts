...
@CustomElement({
    selector: 'newuser-modal',
    template: `<div></div>`,
    style: ``,
    useShadow: true,
})
export class NewUserModel extends HTMLElement {
    ...
    init(): void {
        ...
        const userUrl = `${urlUtil.baseUrl()}/api/modal/NewUser?id=${this.userId}`;
        new HtmlLoader("#NewUserModel").load(userUrl);
