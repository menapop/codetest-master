...
@CustomElement({
    selector: 'deleteuser-modal',
    template: `<div></div>`,
    style: ``,
    useShadow: true,
})
export class DeleteUserModel extends HTMLElement {
    ...
    init(): void {
        ...
        const userUrl = `${urlUtil.baseUrl()}/api/modal/DeleteUser?id=${this.userId}`;
        new HtmlLoader("#DeleteUserModel").load(userUrl);
